...
TERMINAL = ["kitty", "-e"]  # or ['st', '-e'] or something like that
################################################################################
...
CONFIG = {
    ...
    "SEARCH_ENGINE_NAME": {"google": "Google", "duckduckgo": "DuckDuckGo"},
    "SEARCH_URL": {
...
def main():
    ...
    if search_string.endswith("!"):
        ...
        url = CONFIG["SEARCH_URL"][SEARCH_ENGINE] + urllib.parse.quote_plus(
            search_string
        )
        sp.Popen(
            CONFIG["BROWSER_PATH"][BROWSER] + [url],
            stdout=sp.DEVNULL,
            stderr=sp.DEVNULL,
            shell=False,
        )
